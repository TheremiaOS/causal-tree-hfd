...
if __name__ == "__main__":
    ...
    shap_values = rf_explainer.shap_values(X, check_additivity=False)
    shap_interaction_values = rf_explainer.shap_interaction_values(X)

    print("Computing prediction performance...")
    tau1 = .5 * rho/(1 + rho**2) * (X[:, 0]**2 - 1)
    tau2 = .5 * rho/(1 + rho**2) * (X[:, 1]**2 - 1)
    tau12 = .5 * (X[:, 0] * X[:, 1] - rho/(1 + rho**2) * (X[:, 0]**2 + X[:, 1]**2) + rho * (1 - rho**2) / (1 + rho**2))
    ...
    fig.savefig("output/figures/treehfd_simulated_data_2.png")
    ...
    fig.savefig("output/figures/main_components_exp_2.png")
    plt.show()
